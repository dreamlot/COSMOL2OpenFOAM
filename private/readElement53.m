...
%                           of each element
% 
% Ningyu Wang, March 2020
function [elem_data] = readElement53( fp )
% Read ID
while( true ) 
    tmp_line = fgetl(fp) ;
%     pause
...
%     if endsWith(tmp_line,'# number of nodes per element')
    if ~isempty(strfind(tmp_line,'# number of vertices per element'))
        [elem_data.n_nodes] ...
            = sscanf(tmp_line,'%d # number of vertices per element') ;
        break ;
    end
end
